#!/usr/bin/env bash
# setup-was2.sh — Simulate WAS Managed Node 2 (AppServer2)
set -euo pipefail

echo ">>> Configuring Managed Node: ${NODE_NAME} (AppServer2)"

WAS_PROFILE="${WAS_HOME}/profiles/AppSrv02"

# --- Directory structure matching a real WAS ND managed node ---
su - wasadmin -c "
mkdir -p ${WAS_PROFILE}/{bin,config,logs,properties,installedApps}
mkdir -p ${WAS_PROFILE}/config/cells/${CELL_NAME}/nodes/${NODE_NAME}/servers/AppServer2
mkdir -p ${WAS_PROFILE}/logs/AppServer2
mkdir -p ${WAS_PROFILE}/installedApps/${CELL_NAME}
mkdir -p ${WAS_HOME}/bin
"

# --- Simulated server.xml for AppServer2 ---
su - wasadmin -c "cat > ${WAS_PROFILE}/config/cells/${CELL_NAME}/nodes/${NODE_NAME}/servers/AppServer2/server.xml << 'XML'
...
<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<serverindex hostName=\"nexus-was2.${DOMAIN}\">
  <serverEntries serverName=\"AppServer2\" serverType=\"APPLICATION_SERVER\">
    ...
    <specialEndpoints endPointName=\"SOAP_CONNECTOR_ADDRESS\">
      <endPoint host=\"nexus-was2.${DOMAIN}\" port=\"8880\"/>
    </specialEndpoints>
    <specialEndpoints endPointName=\"BOOTSTRAP_ADDRESS\">
      <endPoint host=\"nexus-was2.${DOMAIN}\" port=\"9811\"/>
    </specialEndpoints>
  </serverEntries>
  <serverEntries serverName=\"nodeagent\" serverType=\"NODE_AGENT\">
    <specialEndpoints endPointName=\"SOAP_CONNECTOR_ADDRESS\">
      <endPoint host=\"nexus-was2.${DOMAIN}\" port=\"8878\"/>
    </specialEndpoints>
  </serverEntries>
</serverindex>
XML"
...
firewall-cmd --permanent --add-port=9811/tcp   # Bootstrap
firewall-cmd --permanent --add-port=8878/tcp   # Node agent SOAP
firewall-cmd --permanent --add-port=7273/tcp   # DCS unicast
firewall-cmd --reload

echo ">>> Managed Node 2 setup complete"

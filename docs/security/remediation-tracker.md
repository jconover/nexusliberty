...
| 4 | MEDIUM | `docker/liberty-app/Dockerfile:22` | Liberty base image tag not pinned to digest | **FIXED** | Pinned to `@sha256:5de151a...` (amd64 digest for `kernel-slim-java17-openj9-ubi-minimal`) |
| 5 | MEDIUM | `openshift/ihs-deployment/deployment.yaml:47` | IHS image uses `:latest` tag | **FIXED** | Changed to `:main`; added `update-manifest` job to `ihs-build.yml` that commits SHA tag back on each push to main |
...
| 10 | MEDIUM | `openshift/github-runner/arc-values.yaml:54` | Runner pod image uses `:latest` tag | **FIXED** | Changed to `:main`; comment documents pinning to SHA tag after each rebuild |
...
*All deferred items have been resolved. No outstanding prerequisites.*
